...
public class JobController {
    ...
    public String index(Model model, int id) {
        Job jobObj = jobData.findById(id);
    ...
    public String add(Model model, @Valid JobForm jobForm, Errors errors) {
        ...
        PositionType newJobPositionType =
                jobData.getPositionTypes().findById(jobForm.getPositionTypeId());
        CoreCompetency newJobCoreCompetency =
                jobData.getCoreCompetencies().findById(jobForm.getCoreCompetencyId());

        Job newJob = new Job(newJobName, newJobEmployer, newJobLocation, newJobPositionType,
                newJobCoreCompetency);

        jobData.add(newJob);
        model.addAttribute("jobObj", newJob);
